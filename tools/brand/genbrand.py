...
FONTS = os.path.join(ROOT, "tools", "images", "fonts")
# every brand SVG source lands in docs/brand/svg; only the few the site uses are copied to public/brand
OUT = os.path.join(ROOT, "docs", "brand", "svg")
SHIPPED = ["fluxmigrate-lockup-horizontal.svg", "fluxmigrate-mark.svg", "fluxmigrate-mark-white.svg"]
os.makedirs(OUT, exist_ok=True)
FONT = os.path.join(FONTS, "InterDisplay-SemiBold.otf")
...
def og_card():
    ...
    tag_cap = 23.0
    fs_t = tag_cap / 0.7275
    d2, s2, adv2, _ = wordmark(os.path.join(FONTS, "Inter-Medium.otf"), "Cloud  ·  DevOps  ·  SRE  ·  Platform Engineering", fs_t, 0.01)
    mark_s = 2.9
...
# copy the files the website actually loads into public/brand
import shutil
pub = os.path.join(ROOT, "public", "brand")
os.makedirs(pub, exist_ok=True)
for name in SHIPPED:
    shutil.copyfile(os.path.join(OUT, name), os.path.join(pub, name))
print("shipped to public/brand:", ", ".join(SHIPPED))
